...
SQLAlchemy relationships use `lazy="noload"` by default to prevent implicit queries and async greenlet errors. This means relationships return `None` unless explicitly loaded with `selectinload()` or similar. List endpoints support an `expand` query parameter to opt into loading related entities—check the expand values in the service layer and add the appropriate eager load options to the query.
